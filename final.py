...
with st.sidebar:
    ...
    # Add tabs for different configuration options
    config_option = st.radio(
        "Select Option",
        ["Add Speaker", "Manage Speakers"],
        key="config_option"
    )
    
    if config_option == "Add Speaker":
        st.write("### Add New Speaker")
        new_speaker_name = st.text_input("Enter new speaker name").lower()
        new_speaker_audio = st.audio_input("Record reference audio")

        if new_speaker_name and new_speaker_audio and st.button("Add Speaker"):
            try:
    ...
    elif config_option == "Manage Speakers":
        st.write("### Current Speakers")
        current_speakers = list(st.session_state['speaker_embeddings'].keys())
        
        for speaker in current_speakers:
            col1, col2 = st.columns([3, 1])
            with col1:
                st.write(f"**{speaker.title()}**")
            with col2:
                if speaker not in ['devesh', 'rakshit']:  # Prevent deletion of original speakers
                    if st.button("🗑️", key=f"delete_{speaker}"):
                        del st.session_state['speaker_embeddings'][speaker]
                        st.rerun()
...
# Display current known speakers
